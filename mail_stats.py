...
postfix_log       = '/var/log/mail/fw2-mail.log'
...
def parse_line_with_message_id(list):
    ...
    try:
        ...
        file = pathlib.Path(temp_dir, msg_id)
...
def parse_line_with_from(list):
    ...
    try:
        ...
        if not list[position_queue_id + 2].startswith('status=expired'):
            ...
            loaded_dict = load_dict_from_file(pathlib.Path(temp_dir, msg_id))
            ...
            save_dict_to_file(loaded_dict, pathlib.Path(temp_dir, msg_id))
        else:
            pass
    except:
        global number_lines_with_exception
...
def parse_line_with_to(list):
    try:
        ...
        if test_file_exist(pathlib.Path(temp_dir, msg_id)):
            loaded_dict = loaded_dict = load_dict_from_file(pathlib.Path(temp_dir, msg_id))
